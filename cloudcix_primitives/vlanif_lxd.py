"""
Primitive for VLAN tagged interfaces (LXD only)
"""
# stdlib
...
def build(
    ...
    up_script_path = f'/usr/local/bin/vlanif_lxd_br{vlan}_up.sh'
    down_script_path = f'/usr/local/bin/vlanif_lxd_br{vlan}_down.sh'
    service_file_path= f'/etc/systemd/system/vlanif_lxd_br{vlan}.service'

    # Define message
    messages = {
        1000: f'Successfully created and started vlanif_lxd_br{vlan}.service on kvm {host}.',
        1001: f'vlanif_lxd_br{vlan}.service already exists on kvm {host}',
    ...
    # Templates
    # down script
    template = JINJA_ENV.get_template('vlanif_lxd/down.sh.j2')
    ...
    # up script
    template = JINJA_ENV.get_template('vlanif_lxd/up.sh.j2')
    template_verified, template_error = check_template_data(template_data, template)
    if not template_verified:
    ...
    # service file
    template = JINJA_ENV.get_template('vlanif_lxd/interface.service.j2')
    template_verified, template_error = check_template_data(template_data, template)
    if not template_verified:
...
def read(
    ...
    # Define message
    messages = {
        1200: f'Successfully read vlanif_lxd_br{vlan}.service on kvm {host}.',
        1201: f'vlanif_lxd_br{vlan}.service does not exists on kvm {host}',
...
def scrub(
    ...
    # Define message
    messages = {
        1100: f'Successfully scrubbed vlanif_lxd_br{vlan}.service on kvm {host}.',
        1101: f'vlanif_lxd_br{vlan}.service does not exists on kvm {host}',
